...
public class PahoOpcUaMqttClientImpl implements OpcUaMqttClient {
     ...
     */
    public void publish(String brokerUrl, String topic, byte[] payload) throws Exception {
        if (brokerUrl == null || brokerUrl.trim().isEmpty()) {
            throw new IllegalArgumentException("Broker URL must not be null/blank");
        }
        if (topic == null || topic.trim().isEmpty()) {
        ...
        if (isCertificateAuthConfigured()) {
            ...
            publish(brokerUrl, topic, payload, this.username, this.password);
        } else {
            // Use anonymous connection
            publish(brokerUrl, topic, payload, null, null);
        }
    }

    @Override
    public void subscribe(String brokerUrl, String topic, String username, String password) throws Exception {
        if (brokerUrl == null || brokerUrl.trim().isEmpty()) {
            throw new IllegalArgumentException("Broker URL must not be null/blank");
        }
        if (topic == null || topic.trim().isEmpty()) {
    ...
    @Override
    public void subscribe(String brokerUrl, String topic) throws Exception {
        if (brokerUrl == null || brokerUrl.trim().isEmpty()) {
            throw new IllegalArgumentException("Broker URL must not be null/blank");
        }
        if (topic == null || topic.trim().isEmpty()) {
        ...
        if (isCertificateAuthConfigured()) {
            ...
            // Use stored username/password credentials
            subscribe(brokerUrl, topic, this.username, this.password);
        } else {
            // Use anonymous connection
            subscribe(brokerUrl, topic, null, null);
        }
    }

    @Override
    public void setCallback(String brokerUrl, org.eclipse.paho.client.mqttv3.MqttCallback callback, String username, String password) throws Exception {
        if (brokerUrl == null || brokerUrl.trim().isEmpty()) {
            throw new IllegalArgumentException("Broker URL must not be null/blank");
        }
        if (callback == null) {
    ...
    @Override
    public void setCallback(String brokerUrl, org.eclipse.paho.client.mqttv3.MqttCallback callback) throws Exception {
        if (brokerUrl == null || brokerUrl.trim().isEmpty()) {
            throw new IllegalArgumentException("Broker URL must not be null/blank");
        }
        if (callback == null) {
        ...
        if (isCertificateAuthConfigured()) {
            ...
            // Use stored username/password credentials
            setCallback(brokerUrl, callback, this.username, this.password);
        } else {
            // Use anonymous connection
            setCallback(brokerUrl, callback, null, null);
        }
    }

    @Override
    public void disconnect(String brokerUrl) throws Exception {
        if (brokerUrl == null || brokerUrl.trim().isEmpty()) {
            throw new IllegalArgumentException("Broker URL must not be null/blank");
        }
